/**
 * rail_booking.js
 * Final version: Reads all configuration from a .env file.
 */
...
(async function main() {
  try {
    ...
    const CONFIG = {
      ...
      SEAT_CLASS: process.env.SEAT_CLASS || "S_CHAIR",
      NEED_SEATS: parseInt(process.env.NEED_SEATS, 10) || 1,
      TRAIN_NAME: process.env.TRAIN_NAME || "",
      // Read comma-separated values from .env and convert to an array
      PREFERRED_COACHES: process.env.PREFERRED_COACHES
        ? process.env.PREFERRED_COACHES.split(",").map((c) => c.trim())
        : [],
